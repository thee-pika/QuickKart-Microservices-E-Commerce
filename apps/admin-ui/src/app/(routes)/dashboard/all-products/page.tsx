...
import DeleteConfirmationModal from 'apps/admin-ui/src/shared/components/modals/delete-conformation-modal';
...
const fetchProducts = async (token: string) => {
  const res = await axiosInstance.get("/admin/api/get-all-products", {
    headers: {
    ...
    () => [
      {
        ...
        cell: ({ row }: any) => (
          <Image
            src={row?.original?.images?.length > 0 ? row.original.images[0]?.file_url : "https://ik.imagekit.io/m3hqvlyteo/products/product-1755878070303_IskcCflfn.jpg?updatedAt=1755878073820"}
    ...
    <div className="p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <Link
          ...
          className="flex items-center gap-2 bg-[#7d2ebf] hover:bg-[#401465] text-white px-4 py-2 rounded-lg shadow"
        >
          <Plus size={18} />
          Add Product
        </Link>
      ...
      <div className="overflow-x-auto rounded-lg border border-gray-700">
        {isLoading ? (
          <p className="text-gray-400 p-4">Loading Products ...</p>
        ) : (
