...
export const recommendedProducts = async (
  ...
  const userTensorAll = tf.tensor1d(
    new Array(Object.keys(productMap).length).fill(userMap[userId] ?? 0),
    'int32'
  );
  ...
  const recommendProducts: RecommendationProduct[] = Object.keys(productMap)
    .map((productId, index) => ({
      productId,
